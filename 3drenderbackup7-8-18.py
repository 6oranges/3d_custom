...
class Camera:
    ...
    def drawline(self,point1,point2,tex1,tex2,texture):
        ...
        currentv=tex1[1]
        if longest > 0:
            spreaddistance=(point2.distancetocamera-point1.distancetocamera)/longest
            spreadu=(tex2[0]-tex1[0])/longest
            spreadv=(tex2[1]-tex1[1])/longest
        else:
            spreaddistance = 0
            spreadu=0
            spreadv=0
        for i in range(longest+1):
            if x>=0 and x<self.width and y>=0 and y<self.height:
                ...
                if self.zbuffer[y][x]<currentdistance:
                    ...
                    try: # 0.005
                        self.surface.set_at((x,y),texture.get_at((int(currentu),int(currentv))))
                    except Exception as e:
                        self.surface.set_at((x,y),(0,255,0))
                self.texbuffer[y][x][0]=currentu
                self.texbuffer[y][x][1]=currentv
    ...
    def drawtriangle(self,triangle):
        ...
        if point1.distancetocamera and point2.distancetocamera and point3.distancetocamera:
            r=triangle.get_rect()
            ...
            if rectcollide(r,(0,0,self.width,self.height)):
                ...
                self.drawline(point1,point2,tex1,tex2,texture)
                self.drawline(point2,point3,tex2,tex3,texture)
                self.drawline(point3,point1,tex3,tex1,texture)
                r=self.croprect(r)
                errors[1]=r
                # Fill in triangle
                startx = 0
                ...
                for y in range(r[1],r[3]+r[1]):
                    lookfor = 0
                    rowdis = self.pbuffer[y]
                    for x in range(r[0],r[2]+r[0]):
                        if lookfor == 0:
                            if rowdis[x]!=INVERSEMAXDISTANCE:
                                lookfor = 1
                                startx = x
                        elif lookfor == 1:
                            if rowdis[x]==INVERSEMAXDISTANCE:
                    ...
                    if lookfor == 3:
                        rowtex = self.texbuffer[y]
                        starttex=rowtex[startx]
                        endtex=rowtex[endx]
                        rowzbuff = self.zbuffer[y]
                        spreadsize = float(endx-startx)
                        currentdistance = rowdis[startx]
                        currentu=starttex[0]
                        currentv=starttex[1]
                        spreaddist = (rowdis[endx]-currentdistance)/spreadsize
                        spreadu = (endtex[0]-currentu)/spreadsize
                        spreadv = (endtex[1]-currentv)/spreadsize
                        for x in range(startx,endx):
                            # this if is slow
                            if rowzbuff[x]<currentdistance:
                                rowzbuff[x]=currentdistance # .001
                                try: # 0.005
                                    self.surface.set_at((x,y),texture.get_at((int(currentu),int(currentv))))
                                except Exception as e:
                                    self.surface.set_at((x,y),(0,255,0))
##                            self.setpoint(x,y,triangle.texture,currentdistance,currenttex)
                ...
                self.eraseline(point1,point2)
                self.eraseline(point2,point3)
                self.eraseline(point3,point1)
...
def main():
    ...
    buttons=set()
    speed=64
    textures,points,triangles=read_3mf("fish")
